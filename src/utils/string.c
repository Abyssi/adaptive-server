...
char ***split_parameters(char *header, int *count)
{
    char ***parameters;
    
    int rc;
    char **first_components;
    int first_components_count;
    int second_components_count;
    
    rc = str_split(header, ',', &first_components, &first_components_count);
    if(rc) printf("Error: %s errno: %d \n", strerror(errno), errno);
    
    parameters = malloc(first_components_count * sizeof( char **));
    
    for (int i = 0; i < first_components_count; i++) {
        
        if (index_of_char(first_components[i], ';') == -1)
        {
            parameters[i] = malloc(2 * sizeof(char *));
            parameters[i][0] = (char *)string_copy(first_components[i]);
            parameters[i][1] = (char *)string_copy("q=1");
        }
        else
        {
            rc = str_split(first_components[i], ';', &parameters[i], &second_components_count);
            if(rc) printf("Error: %s errno: %d \n", strerror(errno), errno);
        }
        
    }
    
    *count = first_components_count;
